$(document).ready(function() {
  ...
  var squares = [
    ...
    "Standard orbit::",
    "Camel toe::",
    "Someone interrupts Data::",
    "Wesley is annoying::",
    "Picard loses his patience::",
    "Troi senses something...::",
    "Riker makes interspecies contact::",
    "La Forge chuckles::",
    "Moral conundrum::",
    "Medical emergency::",
    "Worf is angry::",
    ...
    "Ship separates::",
    "Tea, Earl Grey::",
    "Guinan imparts wisdom::",
    "Famous guest star::",
    "You have the bridge::",
    "Assemble an away team::",
    "Transporter or Replicator problem::"
  ];
  ...
  function freeDab() {
    ...
    if ( dabColor == 'random' ) {
      $('#cell12').addClass(randomDab[Math.floor(Math.random()*randomDab.length)]).html('To boldly go where no bingo has gone before...').attr('title', ""); 
    } else {
      $('#cell12').addClass(dabColor).html('To boldly go where no bingo has gone before...').attr('title', "");
    }
  }
  ...
  function bingoCheck() {
    ...
    for ( i = 0; i < 21; i += 5 ) {
      ...
      if (containsAll(["cell"+i,"cell"+(i+1),"cell"+(i+2),"cell"+(i+3),"cell"+(i+4)], isDabbed)) {
        if ($.inArray("row"+i,bingoRow) == -1) {
          bingoRow.push("row"+i);
          $('#scoreRow').html(bingoRow.length);
          alert("BINGO! You will be assimilated. Resistance is futile...");
        }
    ...
    for ( i = 0; i < 5; i++ ) {
      ...
      if (containsAll(["cell"+i,"cell"+(i+5),"cell"+(i+10),"cell"+(i+15),"cell"+(i+20)], isDabbed)) {
        if ($.inArray("col"+i,bingoCol) == -1) {
          bingoCol.push("col"+i);
          $('#scoreCol').html(bingoCol.length);
          alert("tlhIngan Hol vIjatlhtaHvIS, chay' BINGO vIjatlh?");
        } 
    ...
    if (containsAll(["cell4","cell8","cell12","cell16","cell20"], isDabbed)) {
      if ($.inArray("diag4",bingoDiag) == -1) {
        ...
        $('#scoreDiag').html(bingoDiag.length);
        alert("Rules of Acquisition Number 55: Take joy from profit and profit from joy.");
      }
    } else {
      if ($.inArray("diag4",bingoDiag) !== -1) {
    ...
    if ((bingoRow.length == 5) && (bingoCol.length == 5) && (bingoDiag.length == 2)) {
      alert("Welcome to the United Federation of Planets");
    }
  }
